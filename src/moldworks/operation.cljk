(ns moldworks.operation
  "OperationActor -- one injection-molding-plant manufacturing operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Molding Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Clamp-Force Governor
  ...
  One graph run = one injection-molding-plant manufacturing operation
  ...
  (:require [langgraph.graph :as g]
            ...
            [moldworks.moldworksadvisor :as moldworksadvisor]
...
(defn build
  ...
  `moldworks.store/Store`).
  opts:
    :advisor      -- a `moldworks.moldworksadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (moldworksadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Molding Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (moldworksadvisor/-advise advisor store request)]
            {:proposal p :audit [(moldworksadvisor/trace request p)]})))

      ;; Clamp-Force Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
