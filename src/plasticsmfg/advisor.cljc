(ns plasticsmfg.advisor
  "PlasticsAdvisor -- the *contained intelligence node* for the
  plastics-products plant-operations coordination actor.

  It normalizes production-batch patches (resin-type/weight/reject-
  rate), drafts an injection-molding/extrusion/blow-molding-line
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound-plastics-product-
  ...
  a real molding/extrusion-line actuation or freight dispatch. Every
  output is censored downstream by `plasticsmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `plasticsmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; extrusion/blow-molding-line-control
                                 ; effect
  ...
  contract every caller of this actor agrees to) -- `plasticsmfg.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [plasticsmfg.registry :as registry]
            [plasticsmfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's resin-type,
  weight, reject-rate or verification status. High confidence, low
...
(defn- schedule-maintenance
  "Draft an injection-molding/extrusion/blow-molding-line maintenance-
  ...
  rationale, but `plasticsmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a materials-safety/fume-hazard/equipment-safety concern.
  ...
  technicality). See `plasticsmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `plasticsmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound-plastics-product-shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed weight -- it does NOT invent one, and
  `plasticsmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたはプラスチック製品工場プラント運用コーディネーターの助言者です。"
       ...
       "射出成形機・押出機・ブロー成形機等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `plasticsmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :plasticsmfg-advisor-proposal
   :op         (:op request)
