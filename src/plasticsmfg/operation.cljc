(ns plasticsmfg.operation
  "PlasticsOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PlasticsAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Plastics Plant Operations
  ...
  (:require [langgraph.graph :as g]
            ...
            [plasticsmfg.advisor :as advisor]
            [plasticsmfg.governor :as governor]
            [plasticsmfg.phase :as phase]
...
(defn build
  "Compiles a PlasticsOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `plasticsmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PlasticsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Plastics Plant Operations Governor -- independent censor.
      (g/add-node :govern
