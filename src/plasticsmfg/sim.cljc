(ns plasticsmfg.sim
  ...
  production weight, a proposal that tries to ACTUATE the molding/
  extrusion line directly (permanently blocked, no override), a
  double-schedule of the same maintenance window, a production-batch
  patch with a fabricated resin type, and a production-batch patch
  with an implausible reject-rate reading.
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:resin-type :pp :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, injection-molder -- escalates, approve) ==")
    ...
    (let [r (exec-op actor "t3"
                      {:op :flag-safety-concern :effect :propose :subject "concern-1"
                       :value {:equipment-id "equip-001" :severity :moderate
                               :description "射出成形機付近の樹脂ヒュームの臭気上昇"}}
                      coordinator)]
      (println r)
      (println "-- human plant supervisor approves --")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-injection-molder :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered extruder -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on equip-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated resin-type -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:resin-type :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible reject-rate reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:reject-rate-percent 999.0}}
                       coordinator))
