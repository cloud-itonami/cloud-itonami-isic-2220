(ns plasticsmfg.store
  "SSoT for the plastics-products plant-operations coordination actor,
  ...
  Decision 1, this vertical is self-contained (no external plastics-
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             resin-type/weight/reject-rate record.
    ...
    - `equipment`         -- an injection-molding/extrusion/blow-
                             molding unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled injection-molding/extrusion/
                             blow-molding-line maintenance-window DRAFT
                             against a piece of equipment
                             (`plasticsmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound-plastics-product-
                             shipment DRAFT (`plasticsmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (plasticsmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (plasticsmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :resin-type :pp :material "Polypropylene Homopolymer"
                :weight-kg 50000.0 :reject-rate-percent 1.5
                :verified? true :registered? true
                :shipped-weight-kg 10000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :resin-type :abs :material "ABS Copolymer"
                :weight-kg 8000.0 :reject-rate-percent 0.8
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :resin-type :pe-hdpe :material "HDPE Blow-Molding Grade"
                :weight-kg 60000.0 :reject-rate-percent 2.2
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :injection-molder
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :extruder
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `plasticsmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `plasticsmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered injection-molder
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  extruder unit (blocks any maintenance scheduling against it) -- so
