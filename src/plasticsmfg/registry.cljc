(ns plasticsmfg.registry
  "Pure-function domain logic for the plastics-products plant-operations
  ...
  pre-existing `kotoba-lang/plasticsmfg`-style capability library to
  ...
  proposal's own self-reported weight/status when the inputs needed to
  ...
  coordinated shipment), not the act of actuating injection-molding/
  extrusion/blow-molding-line equipment or dispatching a real freight
  carrier (this actor NEVER does either -- see README `What this actor
  does NOT do`).")

;; ----------------------------- constants -----------------------------

(def valid-resin-types
  "The closed set of resin-type values a production-batch record may
  declare (common thermoplastics processed via injection molding,
  extrusion, or blow molding). Anything else is a fabricated/
  unrecognized resin type -- the governor HARD-holds rather than let
  an invented resin type pass through."
  #{:pp :pe-hdpe :pe-ldpe :pvc :ps :abs :pet :pc :nylon})

(def reject-rate-min-percent
  "Physical floor for a batch's own reject/defect-rate reading (zero
  rejects is the best possible outcome, never negative)."
  0.0)

(def reject-rate-max-percent
  "Physical ceiling for a batch's own reject/defect-rate reading -- a
...
(defn batch-verified?
  ...
  (i.e. its resin-type/weight/reject-rate claims have actually been
...
(defn resin-type-valid?
  "Is `resin-type` one of the closed, known resin-type values? nil/
  blank is treated as invalid (a production-batch patch must declare a
  real resin type, not omit it silently)."
  [resin-type]
  (contains? valid-resin-types resin-type))

(defn reject-rate-valid?
  "Is `percent` a physically plausible batch reject/defect-rate
  reading? Rejects nil, non-numbers, negative values, and values
  beyond `reject-rate-max-percent` -- a fabricated or sensor-error
  reading, never let through as a real batch fact."
  [percent]
  (and (number? percent)
       (>= (double percent) reject-rate-min-percent)
       (<= (double percent) reject-rate-max-percent)))
...
(defn register-maintenance
  ...
  injection-molding/extrusion/blow-molding-line maintenance window
  against a verified, registered piece of equipment. Pure function --
  does not actuate the molding/extrusion line or execute any
  maintenance; it builds the RECORD a plant coordinator would keep.
  `plasticsmfg.governor` independently re-verifies the equipment's own
  verified/registered ground truth, and permanently blocks any attempt
  to directly actuate the line (see README `Actuation`), before this
...
(defn register-shipment
  ...
  outbound plastics-product shipment against a verified, registered
  production batch. Pure function -- does not dispatch any real
  freight carrier; it builds the RECORD a plant coordinator would
  keep. `plasticsmfg.governor` independently re-verifies the
