(ns plasticsmfg.governor
  "Plastics Plant Operations Governor -- the independent compliance
  layer that earns the PlasticsAdvisor the right to commit. The
  ...
  injection-molding/extrusion/blow-molding line, whether a shipment
  ...
  proposal and becomes direct molding/extrusion-line-equipment
  control, so this MUST be a separate system able to *reject* a
  proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:plastics-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       `:molder/run`) is the 'direct
                                       molding/extrusion-line-equipment
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly actuating the injection-
    ...
    5. Equipment not verified/
       registered                  -- for `:schedule-maintenance`,
                                       ...
                                       are both true
                                       (`plasticsmfg.registry/equipment-
    ...
    8. Shipment weight exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       the batch's own recorded
                                       `:weight-kg`
                                       (`plasticsmfg.registry/shipment-
                                       weight-exceeded?`) -- ground
                                       truth from the batch's own
                                       permanent fields, never a
                                       self-reported weight claim.
    9. Invalid resin-type          -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:resin-type` outside the closed
                                       known set
                                       (`plasticsmfg.registry/resin-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated resin type through.
   10. Invalid reject-rate         -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:reject-rate-percent` that is
                                       not a physically plausible
                                       reading
                                       (`plasticsmfg.registry/reject-
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [plasticsmfg.registry :as registry]
            [plasticsmfg.store :as store]))
...
(def allowed-proposal-effects
  ...
  extrusion/blow-molding-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct injection-molding/extrusion/blow-molding-line
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は射出成形機・押出機・ブロー成形機等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the molding/extrusion line -- this actor may only
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-actuate-blocked
      :detail "成形/押出ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-resin-type-violations
  "For `:log-production-batch`, if the patch declares a `:resin-type`
  outside the closed known set, reject rather than let a fabricated
  resin type through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [resin-type (:resin-type (:value proposal))]
      (when (and (some? resin-type) (not (registry/resin-type-valid? resin-type)))
        [{:rule :invalid-resin-type
          :detail (str resin-type " は既知の resin-type 値ではない")}]))))

(defn- invalid-reject-rate-violations
  "For `:log-production-batch`, if the patch declares a
  `:reject-rate-percent` that is not a physically plausible reading,
  reject rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rr (:reject-rate-percent (:value proposal))]
      (when (and (some? rr) (not (registry/reject-rate-valid? rr)))
        [{:rule :invalid-reject-rate
          :detail (str rr "% は物理的に妥当な不良率の範囲外")}]))))

(defn check
  "Censors a PlasticsAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-resin-type-violations request proposal)
                           (invalid-reject-rate-violations request proposal)))
