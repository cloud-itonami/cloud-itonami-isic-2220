(ns plasticsmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control molding/extrusion/blow-molding
  ...
  test:
    ...
    PlasticsAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Plastics Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:resin-type :pp}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :pp (:resin-type (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "equip-001" :maintenance-type :screw-inspection
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-injection-molder :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "equip-002" :maintenance-type :die-inspection
                             :scheduled-date "2026-08-01" :actuate-line? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "equip-001" :maintenance-type :line-run-force
...
(deftest invalid-resin-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:resin-type :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-resin-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:resin-type (store/batch db "batch-001"))) "fabricated resin-type never lands in the SSoT")))

(deftest invalid-reject-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:reject-rate-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-reject-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:reject-rate-percent (store/batch db "batch-001"))) "fabricated reject-rate never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "resin fumes near injection molder"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:resin-type :fabricated-resin}} coordinator)
